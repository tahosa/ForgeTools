	...
	public void processCommand(ICommandSender sender, String[] args)
	{
		...
		if(lastCheck.getTime() < new Date().getTime() - (ForgeTools.timeout * 1000) || force || kill || killall)
		{
			...
			for(WorldServer s : server.worldServers)
			{
				...
				if(kill || killall)
					sender.addChatMessage(new ChatComponentText(prefix + itemsDeleted + " items removed from " + s.provider.worldObj.getWorldInfo().getWorldName()+" " + s.provider.getDimensionName()));
				if(details)
					sender.addChatMessage(new ChatComponentText(prefix + worldItemCount + " items in " + s.provider.worldObj.getWorldInfo().getWorldName()+" " + s.provider.getDimensionName()));
			}
		}
		else
		{
		...
		if(details)
		{
			...
			sender.addChatMessage(new ChatComponentText("Top 5 chunks by loose item count:"));
			...
			for (int i = 0; i < sortedList.length && i < 5; ++i)
			{
				...
				sender.addChatMessage(new ChatComponentText(worldName + " " + dimName + " (" + (c.xPosition * 16) + ", " + (c.zPosition * 16) + ") " + cr.getValue() + " items"));
			}
		}
		
		if (!(details || kill || killall)) sender.addChatMessage(new ChatComponentText(total + " loose items in all worlds"));
	}
