...
{	
	...
	public void processCommand(ICommandSender sender, String[] args)
	{
		...
		if(current || dim != null)
		{
			...
			{
				sender.addChatMessage(new ChatComponentText("Invalid option. As the console you are not in a dimension!"));
				return;
			}
			...
			{
				sender.addChatMessage(new ChatComponentText("\u00a74" + dim + " is not a valid dimension id"));
				return;
			}
		...
		else
		{
			...
			double tickPct = (tickMS < 50) ? 100d : Math.round(50d/tickMS * 1000)/10d;
			double tps = (tickMS < 50) ? 20d : Math.round((1000d/tickMS) * 10d) / 10d;
			sender.addChatMessage(new ChatComponentText(textColor(tps) + "Tick: "+tps+" tps ("+tickMS + "ms, "+ tickPct+"%)"));
		}
